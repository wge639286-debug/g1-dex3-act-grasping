...
- 搭建Unitree G1 + Dex3 MuJoCo视觉抓取系统，实现7自由度位姿IK、三指接触控制及头部RGB观测。
- 构建25 Hz示范采集与LeRobot转换流程，采集30条、12,956帧、14D状态/动作和同步RGB数据，并在RTX 4090D上训练ACT策略。
- 实现ACT闭环部署和接触感知评估，定位短执行窗口导致的动作拖延问题，将执行窗口由5步调整为25步；在10个预设位置完成抓取抬升，并在困难位置的X/Y各±2 mm扰动测试中达到10/10成功。
